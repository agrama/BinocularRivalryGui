    ...
    """#version 140
        ...
        void main() {
          ...
          if (pow((texcoord.x - 0.5)*aspect_ratio,2) + pow((texcoord.y - 0.5),2) < pow(mask_radius,2) ){
            if(stimcode == 1){
            gl_FragColor = vec4(low_contrast*gratings_brightness*0.5*sign(sin(texcoord_rotated2.x * 2 * 3.14 * cycles + phase2)) + gratings_brightness, 0,0, 1);
            }
            if (stimcode ==2){
            gl_FragColor = vec4(high_contrast*gratings_brightness*0.5*sign(sin(texcoord_rotated1.x * 2 * 3.14 *  cycles + phase1))+ gratings_brightness, 0, 0, 1);
            }
            if (stimcode==3){
            gl_FragColor = vec4(gratings_brightness,0,0,1);
            }
          }
          else{
...
class MyApp(ShowBase):
    ...
    def increase_brightness(self):
        self.gratings_brightness +=0.05
        print(self.gratings_brightness)
        self.cardnode.setShaderInput('gratings_brightness',self.gratings_brightness)
